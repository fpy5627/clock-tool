...
export async function generateMetadata({
  ...
  // 构建页面URL
  const pagePath = "/alarm";
  const pageUrl = getCanonicalUrl(pagePath, locale);
  const languages = getHreflangLanguages(pagePath);

  // 获取翻译文本
  const title = t("clock.modes.alarm") || "Alarm Clock";
  const description = t("metadata.description") || "Free online alarm clock with multiple alarms. Set daily, weekday, weekend, or one-time alarms with custom sounds.";
 ...
 * @returns 返回布局组件
 */
export default async function AlarmLayout({
  children,
  params,
}: {
