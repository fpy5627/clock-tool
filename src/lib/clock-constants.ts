 ...
 * 声音选项列表
 * 
 * 从 notifySoundMetaList 转换而来，使用实际音频文件
 */
export const SOUND_OPTIONS: SoundOption[] = notifySoundMetaList.map((sound, index) => {
  // 英文名称映射（对于原本是英文名称的选项）
  const nameEnMap: Record<string, string> = {
    'serene-solo-piano-airy-synth-pad-alpha-wave-v2': 'Serene Solo Piano with Airy Synth Pad and Gentle Alpha Wave V2',
    'peaceful-piano-instrumental-studying-focus': 'Peaceful Piano Instrumental for Studying and Focus',
    'pulse-synthwave-dude': 'Pulse Synthwave Dude',
  };
  ...
  return {
    ...
    key: sound.id,
    name: sound.name,
    nameEn: nameEnMap[sound.id] || sound.name, // 如果有英文名称映射则使用，否则使用中文名称
    description: sound.desc,
    descriptionEn: sound.desc, // 如果没有英文描述，使用中文描述
    type: 'file', // 标记为文件类型
    mode: 'file',
    popularity: 100 - index, // 按顺序设置人气值，第一个最高
  };
});
...
export const WORLD_CITIES = [
  ...
  { key: 'hongkong', timezone: 'Asia/Hong_Kong', offset: 8, weatherCode: '116', temp: 21, countryKey: 'china' },
