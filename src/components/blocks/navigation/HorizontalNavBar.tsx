...
import { Timer, Clock, AlarmClock, Globe, HelpCircle, LucideIcon } from 'lucide-react';
...
/**
 * 水平导航栏组件
 * 用于在页面顶部显示水平导航链接（Timer、Stopwatch、Alarm、World Clock、FAQ）
 * 
 * @param currentPage - 当前页面ID，用于高亮显示
 * @param includeFaq - 是否包含FAQ链接
...
export default function HorizontalNavBar({ 
  ...
  if (includeFaq) {
    navItems.push({ id: 'faq', path: 'faq', labelKey: 'faq', icon: HelpCircle });
  }
        ...
        <div className="flex items-center justify-center gap-1 sm:gap-2 py-3 px-2 sm:px-4">
          {navItems.map((item) => {
            ...
            const isActive = currentPage === item.id || 
              (item.id === 'faq' && pathname.includes('/faq')) || 
              (!item.id.includes('faq') && pathname.includes(item.path));
